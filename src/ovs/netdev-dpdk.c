...
//static int dpdk_veth_dev_init(struct netdev_dpdk *dev) OVS_REQUIRES(dpdk_mutex);

//static int netdev_dpdk_veth_construct(struct netdev *netdev_);
//static void netdev_dpdk_veth_destruct(struct netdev *netdev_);
//static int netdev_dpdk_veth_rxq_recv(struct netdev_rxq *rxq_, struct dp_packet **packets, int *c);
//static int netdev_dpdk_veth_send(struct netdev *netdev_, int qid OVS_UNUSED, struct dp_packet **pkts, int cnt, bool may_steal);
//static int netdev_dpdk_veth_get_stats(const struct netdev *netdev,struct netdev_stats *stats);
//void kni_dpdk_set_mac(struct netdev_dpdk *dev);
 ...
 **/
static int
netdev_dpdk_kni_construct(struct netdev *netdev)
{
	struct netdev_dpdk *dev = netdev_dpdk_cast(netdev);
	struct rte_kni_conf conf;
	static int rte_kni_port_id = 1;
	...
	conf.group_id = (uint16_t)rte_kni_port_id; /* currently group id is hardcoded */
	...
	if(veth == NULL)
	{
		VLOG_ERR("Failed to allocate KNI interface");
		return ENOMEM;
	}
	
	rte_kni_port_id ++; /* increment the port id for the next kni interface */
	dev->veth = veth;
	
	ovs_mutex_lock(&dpdk_mutex);
	err = common_construct(netdev, -1, DPDK_DEV_ETH, SOCKET0);
	ovs_mutex_unlock(&dpdk_mutex);
	//rte_spinlock_init(&netdev->tx_q->tx_lock); /* using same structure host_tx_lock for kni */

	return err;
}

static void
netdev_dpdk_kni_destruct(struct netdev *netdev)
{
	struct netdev_dpdk *dev = netdev_dpdk_cast(netdev);
	ovs_mutex_lock(&dev->mutex);
...
static int
netdev_dpdk_kni_rxq_recv(struct netdev_rxq *rxq_,
			struct dp_packet_batch *batch)
{
	...
	{
		/* receive and free */
		nb_rx =  rte_kni_rx_burst(veth, (struct rte_mbuf **)batch->packets, PORT_VETH_RX_BURST_SIZE);
...
static int
netdev_dpdk_kni_send(struct netdev *netdev, int qid,
                       struct dp_packet_batch *batch,
                       bool may_steal, bool concurrent_txq OVS_UNUSED)
{
	struct netdev_dpdk *dev = netdev_dpdk_cast(netdev);
	...
	unsigned int tx_pkts = 0, dropped_pkts = 0, i, cnt=batch->count;
	if (!thread_is_pmd()) {
		goto out;
	}
	...
	rte_spinlock_lock(&dev->tx_q->tx_lock);
	for (i=0;i<cnt;i++)
	{
		if (cur_pkts[i]->data_off < RTE_PKTMBUF_HEADROOM)
			++dropped_pkts;
		else if (rte_kni_tx_burst(veth, (struct rte_mbuf **) &cur_pkts[i], qid))
			++tx_pkts;
		else
			++dropped_pkts;
	}
	rte_spinlock_unlock(&dev->tx_q->tx_lock);

	/* statistics for the no of packets sent */
	rte_spinlock_lock(&dev->stats_lock);
	dev->stats.tx_packets += tx_pkts;
	dev->stats.tx_dropped += dropped_pkts;
...
static int
netdev_dpdk_kni_get_stats(const struct netdev *netdev,
                            struct netdev_stats *stats)
{
	struct netdev_dpdk *dev = netdev_dpdk_cast(netdev);
...
static const struct netdev_class dpdk_kni_class =
	...
	"dpdkkni",
	netdev_dpdk_class_init,
	netdev_dpdk_kni_construct,
	netdev_dpdk_kni_destruct,
	NULL,
	NULL,
	netdev_dpdk_kni_send,
	NULL,
	netdev_dpdk_kni_get_stats,
	NULL,
	NULL,
	NULL,
	netdev_dpdk_kni_rxq_recv);
